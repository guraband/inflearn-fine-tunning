...
├── compare_models.py       # 원본 vs 파인튜닝 모델 비교
├── main.py                 # 간단한 실행 스크립트
...
### 모델 비교 (`compare_models.py`)

원본 DistilBERT 모델과 파인튜닝된 모델의 성능을 비교하는 스크립트입니다.

#### 주요 기능:

1. **자동 테스트**: 10개의 다양한 테스트 문장으로 성능 비교
2. **대화형 테스트**: 사용자가 직접 입력한 텍스트로 비교
3. **상세 분석**: 카테고리별, 정확도별 분석 제공
4. **성능 개선 측정**: 파인튜닝 전후 성능 차이 계산
